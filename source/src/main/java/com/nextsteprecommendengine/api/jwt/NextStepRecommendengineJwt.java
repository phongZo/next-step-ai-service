package com.nextsteprecommendengine.api.jwt;

import com.nextsteprecommendengine.api.utils.ZipUtils;
import lombok.Data;
...
@Slf4j
@Data
public class NextStepRecommendengineJwt implements Serializable {
    ...
    public static NextStepRecommendengineJwt decode(String input){
        NextStepRecommendengineJwt result = null;
        try {
            String[] items = ZipUtils.unzipString(input).split(DELIM,11);
            if(items.length >= 10){
                result = new NextStepRecommendengineJwt();
